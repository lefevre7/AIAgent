...
- **Turns.** `runtime.maxTurnsPerRun` defaults to `"unlimited"` with a no-progress guard (`runtime.maxConsecutiveNudges`, default 3 consecutive unproductive turns → `completion_blocked`). Unproductive = no tool calls, or mixed/rejected completion attempts.
...
- **Vendor settings verified (2026-06-12).** `docs/SMALL_MODELS.md` now ships verified Qwen3-Coder (`0.7/0.8`, official), Qwen3.5 non-thinking-text (`1.0/1.0` + presence_penalty `2.0`) and thinking-mode tables, with Ollama's `num_ctx=4096` default and silent-truncation behavior cited. Top-k, presence_penalty, repetition_penalty, and min_p are not yet threaded through `runtime.modelSettings` — the LM Studio / Ollama Modelfile presets typically encode them. If a future change exposes them, plumb them through `languageModelSettingsSchema` (`src/core/contracts/providers.ts`), both adapters' `buildPayload`, and the loop's request builder in one pass.
