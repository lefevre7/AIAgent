...
- **Status metrics.** `AgentLoop` emits `onStatus` metrics `{ contextWindowPercentage, tokensUsed, elapsedSeconds }` after each model response (context% needs `runtime.modelSettings.contextWindowTokens`). The gateway forwards them in the `gateway.status` event payload; the CLI renders them dimmed on stderr during the loop.
