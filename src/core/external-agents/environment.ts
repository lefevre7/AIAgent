 ...
 * per agent through `passEnv`, which is the point: the operator names the
 * secrets a given CLI may see instead of every child inheriting the whole
 * shell.
 */
 ...
 * operator's shell. Precedence is base allowlist, then `passEnv`, then the
 * agent's explicit `env`, then per-call overrides — most specific wins.
 *
...
export function buildExternalAgentEnvironment(params: {
  ...
  for (const key of params.config.passEnv ?? []) {
    const value = source[key];
    if (typeof value === "string") {
      env[key] = value;
    }
