...
import {
  type WorkspaceBinaryMutationResult,
  type WorkspaceMutationResult,
  type WorkspaceMutationEngine
} from "@/core/workspace";
  ...
  return {
    ...
    async execute(call, context): Promise<RuntimeToolResult> {
      ...
      if (typeof input.base64Content === "string") {
        const result = await params.workspaceEngine.appendFileBytes(
          targetPath,
          decodeBase64Content(input.base64Content)
        );
...
const readFileToolDefinition: ToolDefinition = createWorkspaceDefinition({
  ...
  description:
    "Read a local file from the workspace or any other local path. Text files can be sliced by line range, and binary files are returned as base64 with media metadata.",
  descriptor: {
  ...
  usageGuidance:
    "Use this when you already know the file path. For large text files, request a line window. For binary files, expect base64 output instead of text."
});
...
const listFilesToolDefinition: ToolDefinition = createWorkspaceDefinition({
  ...
  descriptor: {
    ...
    whenToUse: [
      "Use to inspect a directory or a single file path.",
      "Use before read_file when the exact target is uncertain."
    ]
  },
  ...
  sideEffects: ["workspace_read"],
  usageGuidance:
    "Use this to inspect directories or confirm a file target before deeper actions. Recursive listing is useful, but keep maxEntries bounded on large trees."
});
...
const searchPathsToolDefinition: ToolDefinition = createWorkspaceDefinition({
  ...
  descriptor: {
    ...
    whenNotToUse: [
      "Do not use it for content search inside files.",
      "Do not use it when list_files already narrowed the directory."
    ],
    whenToUse: [
      "Use when the exact file path is unknown.",
      "Use before grep_files when you first need candidate files."
    ]
  },
...
const grepFilesToolDefinition: ToolDefinition = createWorkspaceDefinition({
  ...
  description:
    "Search text inside local files using plain-text or regex matching and return matching lines with locations.",
  descriptor: {
    ...
    whenNotToUse: [
      "Do not use it to find files by name only.",
      "Do not use it when you need semantic code search across the repo."
    ],
    whenToUse: [
      "Use for exact text or regex search across local files.",
      "Use before edit_file or apply_patch when locating exact text to change."
    ]
  },
  ...
  sideEffects: ["workspace_read"],
  usageGuidance:
    "Use this to find exact text or regex matches inside files. Keep maxResults bounded, and scope path when the search space is large."
});
...
const writeFileToolDefinition: ToolDefinition = createWorkspaceDefinition({
  ...
  descriptor: {
    ...
    whenNotToUse: [
      "Do not use it for small in-place edits when edit_file or apply_patch is more precise.",
      "Do not use it for appending content; use append_file."
    ],
  ...
  usageGuidance:
    "Use this to create or fully overwrite a file. Prefer apply_patch or edit_file when a smaller targeted change is safer."
});
...
const appendFileToolDefinition: ToolDefinition = createWorkspaceDefinition({
  ...
  descriptor: {
    ...
    sideEffectSummary: "Writes local files and records an undo snapshot.",
    whenNotToUse: [
      "Do not use it when the change belongs in the middle of a file.",
      "Do not use it to replace whole-file contents."
    ],
    whenToUse: [
      "Use for end-of-file additions.",
      "Use when you explicitly need append semantics instead of overwrite semantics."
    ]
  },
  ...
  sideEffects: ["workspace_write"],
  usageGuidance:
    "Use this only for end-of-file additions. For targeted edits elsewhere in a file, prefer edit_file or apply_patch."
});
...
const editFileToolDefinition: ToolDefinition = createWorkspaceDefinition({
  ...
  description:
    "Apply minimal exact-text replacements to a UTF-8 file. This is the lightweight in-place editor, while apply_patch handles broader line-oriented changes.",
  descriptor: {
    approvalNotes: "Approval is required because this tool mutates local files.",
    ...
    sideEffectSummary: "Writes local files and records an undo snapshot.",
    whenNotToUse: [
      "Do not use it on binary files.",
      "Do not use it when the change is easier to express as line patches."
    ],
  ...
  sideEffects: ["workspace_write"],
  usageGuidance:
    "Use this for exact string replacements in text files. If the change is broader or line-oriented, prefer apply_patch."
});
...
const applyPatchToolDefinition: ToolDefinition = createWorkspaceDefinition({
  ...
  description:
    "Apply one or more line-oriented patches to a UTF-8 file. This is the main structured editor for broader local file changes.",
  descriptor: {
    approvalNotes: "Approval is required because this tool mutates local files.",
    examples: [
      "Replace a function body by line range.",
      "Insert new lines at a specific location without rewriting the whole file."
    ],
    purpose: "Apply structured line-based patches to a text file.",
    sideEffectSummary: "Writes local files and records an undo snapshot.",
    whenNotToUse: [
      "Do not use it on binary files.",
      "Do not use it for tiny exact-text replacements when edit_file is enough."
    ],
  ...
  sideEffects: ["workspace_write"],
  usageGuidance:
    "Use this as the main structured editor for multi-line text changes. Provide clear start lines and delete counts, and keep edits scoped to one file per call."
});
...
const diffPreviewToolDefinition: ToolDefinition = createWorkspaceDefinition({
  ...
  descriptor: {
    ...
    whenNotToUse: [
      "Do not use it as a substitute for apply_patch or edit_file.",
      "Do not use it outside a git-backed workflow when you only need current file contents."
    ],
    whenToUse: [
      "Use when reviewing current git changes.",
      "Use when you need a patch-style preview of working tree or staged edits."
    ]
  },
  ...
  sideEffects: ["none"],
  usageGuidance:
    "Use this to inspect git changes, not to propose hypothetical edits. If the target is not in a git repository, the tool will tell you that directly."
});
...
const undoLastEditToolDefinition: ToolDefinition = createWorkspaceDefinition({
  ...
  descriptor: {
    ...
    examples: [
      "Undo the last local edit after a bad patch.",
      "Roll back the most recent file mutation before trying a safer approach."
    ],
    ...
    whenNotToUse: [
      "Do not use it when you need a git revert or repository-wide reset.",
      "Do not use it if you need to undo a specific older change by id."
    ],
    whenToUse: [
      "Use immediately after a bad local mutation.",
      "Use when the latest recorded workspace change should be reverted."
    ]
  },
  ...
  sideEffects: ["workspace_write"],
  usageGuidance:
    "Use this to revert the most recent recorded local file mutation. It is not a git operation and only affects the built-in undo history."
});
...
const undoFileEditToolDefinition: ToolDefinition = createWorkspaceDefinition({
  ...
  descriptor: {
    ...
    examples: [
      "Undo the last change to one file while keeping other recent edits.",
      "Roll back the latest mutation for a binary asset."
    ],
    purpose: "Restore the latest recorded undo snapshot for one local file.",
    sideEffectSummary: "Writes local files by restoring a prior snapshot.",
    whenNotToUse: [
      "Do not use it when you want to revert the latest global edit regardless of file.",
      "Do not use it for git history operations."
    ],
    whenToUse: [
      "Use when only one file should be rolled back.",
      "Use after a failed mutation to a specific file while preserving other newer edits."
    ]
  },
  ...
  sideEffects: ["workspace_write"],
  usageGuidance:
    "Use this to revert the latest recorded mutation for a specific file path. It uses the built-in undo history, not git history."
});
...
function renderMutationResult(
  result: WorkspaceMutationResult | WorkspaceBinaryMutationResult,
  isBinary: boolean
): RuntimeToolResult {
  return {
    display: [
