...
describe("language-model message serialization", () => {
  ...
  test("forwards tool-result images after the whole tool-call run, not between tool results", async () => {
    ...
    expect(openai.map((m) => m.role)).toEqual(["system", "assistant", "tool", "tool", "user"]);
    expect(JSON.stringify(openai[openai.length - 1])).toContain("data:image/png;base64,");

    const ollama = await serializeOllamaMessages(request);
    expect(ollama.map((m) => m.role)).toEqual(["system", "assistant", "tool", "tool", "user"]);
    expect(Array.isArray((ollama[ollama.length - 1] as { images?: unknown[] }).images)).toBe(true);
  });

  test("does not forward tool-result images when supportsVision is false", async () => {
    const uri = await pngUri();
    const assistant: Message = {
      ...message([{ arguments: {}, callId: "call.1", kind: "tool_call", toolName: "shot" }], "assistant"),
  ...
  test("serializes assistant tool calls and paired tool results natively for Ollama", async () => {
    const assistant: Message = {
      ...message(
        [{ arguments: { path: "README.md" }, callId: "call.2", kind: "tool_call", toolName: "read_file" }],
        "assistant"
      ),
...
describe("language-model image URI resolution", () => {
  test("passes data URLs through, reads absolute paths, and infers mime by extension", async () => {
    ...
    expect(
      JSON.stringify(await serializeOllamaMessages(buildRequest([message([{ kind: "image", uri: gif }])])))
    ).toContain("images");
  });
