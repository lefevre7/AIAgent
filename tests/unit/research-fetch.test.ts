...
describe("WebPageFetcher", () => {
  ...
  test("refuses a redirect that points at a private or loopback host (SSRF)", async () => {
    for (const location of ["http://127.0.0.1:8080/admin", "http://169.254.169.254/latest/meta-data/", "http://localhost/", "http://[::1]/"]) {
      const fetchImpl = (async () => new Response(null, { headers: { location }, status: 302 })) as unknown as typeof fetch;
      await expect(new WebPageFetcher({ fetchImpl }).fetch({ url: "https://example.com/redirect" })).rejects.toMatchObject({
        code: "web_fetch_private_host_blocked"
      });
    }
  });
  ...
  test("stops after too many redirects and rejects a redirect without a Location", async () => {
    const looping = (async () => new Response(null, { headers: { location: "https://example.com/loop" }, status: 302 })) as unknown as typeof fetch;
    await expect(new WebPageFetcher({ fetchImpl: looping, maxRedirects: 3 }).fetch({ url: "https://example.com/loop" })).rejects.toMatchObject({
      code: "web_fetch_too_many_redirects"
    });

    const headless = (async () => new Response(null, { status: 302 })) as unknown as typeof fetch;
    await expect(new WebPageFetcher({ fetchImpl: headless }).fetch({ url: "https://example.com/x" })).rejects.toMatchObject({
  ...
  test("refuses hostnames that resolve to private addresses and reports DNS failures", async () => {
    ...
    await expect(
      new WebPageFetcher({ fetchImpl, resolveHost: async () => ["93.184.216.34", "10.0.0.7"] }).fetch({ url: "https://evil.example.com/" })
    ).rejects.toMatchObject({ code: "web_fetch_private_host_blocked" });

    await expect(
      new WebPageFetcher({ fetchImpl, resolveHost: async () => ["::ffff:127.0.0.1"] }).fetch({ url: "https://mapped.example.com/" })
    ).rejects.toMatchObject({ code: "web_fetch_private_host_blocked" });
    ...
    const ok = await new WebPageFetcher({ fetchImpl, resolveHost: async () => ["93.184.216.34", "2606:2800:220:1:248:1893:25c8:1946"] }).fetch({
  ...
  test("classifies private, loopback, link-local, and mapped addresses", () => {
    for (const address of ["127.0.0.1", "10.1.2.3", "172.16.0.1", "172.31.255.255", "192.168.0.1", "169.254.169.254", "100.64.0.1", "0.0.0.0", "224.0.0.1", "::1", "::", "fd12::1", "fc00::1", "fe80::1", "ff02::1", "::ffff:192.168.1.1", "::ffff:c0a8:101"]) {
      expect(isPrivateIpAddress(address), address).toBe(true);
    }
    for (const address of ["8.8.8.8", "93.184.216.34", "172.32.0.1", "100.128.0.1", "2606:4700:4700::1111", "::ffff:8.8.8.8"]) {
